...
3. row와 col만큼 생성된 이중 배열에 곱셈 값을 담는 함수
ㄴ 매개변수 _row, _col
ㄴ return 2중 배열

4. 테이블 row 몇 단까지 출력할지 scan받아 return 하는 함수
ㄴ  return dan : row size
*/

// 몇 단 출력할 지 scan하는 scanner
int DanScanner();

// row와 col값을 받아 int 2중 배열에 값 담는 함수 ( 포인터 리턴)
int* PutResult2DArray(int _row, int _col);

// rval, lval받아 곱셈 값 리턴해 주는 함수
int Multiply(int _lVal, int _rVal);

// 2중 배열을 테이블 형태로 프린트해주는 함수
